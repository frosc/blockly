...
Blockly.ASEBA['math_arithmetic'] = function(block) {
  ...
  var OPERATORS = {
    ...
    'POWER': [null, Blockly.ASEBA.ORDER_COMMA]  // Handle power separately.
  };
...
Blockly.ASEBA['math_single'] = function(block) {
  ...
  if (operator == 'SIN' || operator == 'COS' || operator == 'TAN') {
    arg = Blockly.ASEBA.valueToCode(block, 'NUM',
        Blockly.ASEBA.ORDER_DIVISION) || '0';
  } else {
    arg = Blockly.ASEBA.valueToCode(block, 'NUM',
  ...
  return [code, Blockly.ASEBA.ORDER_DIVISION];
};

Blockly.ASEBA['math_constant'] = function(block) {
  // Constants: PI, E, the Golden Ratio, sqrt(2), 1/sqrt(2), INFINITY.
  var CONSTANTS = {
    'PI': ['Math.PI', Blockly.ASEBA.ORDER_MEMBER],
    'E': ['Math.E', Blockly.ASEBA.ORDER_MEMBER],
    'GOLDEN_RATIO':
        ['(1 + Math.sqrt(5)) / 2', Blockly.ASEBA.ORDER_DIVISION],
    'SQRT2': ['Math.SQRT2', Blockly.ASEBA.ORDER_MEMBER],
    'SQRT1_2': ['Math.SQRT1_2', Blockly.ASEBA.ORDER_MEMBER],
    'INFINITY': ['Infinity', Blockly.ASEBA.ORDER_ATOMIC]
  };
...
Blockly.ASEBA['math_number_property'] = function(block) {
  ...
  var number_to_check = Blockly.ASEBA.valueToCode(block, 'NUMBER_TO_CHECK',
  ...
  if (dropdown_property == 'PRIME') {
    ...
    var functionName = Blockly.ASEBA.provideFunction_(
        'math_isPrime',
        [ 'function ' + Blockly.ASEBA.FUNCTION_NAME_PLACEHOLDER_ + '(n) {',
  ...
  switch (dropdown_property) {
    ...
    case 'NEGATIVE':
      ...
      break;
    case 'DIVISIBLE_BY':
      var divisor = Blockly.ASEBA.valueToCode(block, 'DIVISOR',
          Blockly.ASEBA.ORDER_MODULUS) || '0';
      code = number_to_check + ' % ' + divisor + ' == 0';
      break;
  }
  return [code, Blockly.ASEBA.ORDER_EQUALITY];
};

Blockly.ASEBA['math_change'] = function(block) {
  // Add to a variable in place.
  var argument0 = Blockly.ASEBA.valueToCode(block, 'DELTA',
      Blockly.ASEBA.ORDER_ADDITION) || '0';
...
Blockly.ASEBA['math_on_list'] = function(block) {
  ...
  switch (func) {
    case 'SUM':
      list = Blockly.ASEBA.valueToCode(block, 'LIST',
...
Blockly.ASEBA['math_constrain'] = function(block) {
  ...
  var argument1 = Blockly.ASEBA.valueToCode(block, 'LOW',
      Blockly.ASEBA.ORDER_COMMA) || '0';
  var argument2 = Blockly.ASEBA.valueToCode(block, 'HIGH',
      Blockly.ASEBA.ORDER_COMMA) || 'Infinity';
...
Blockly.ASEBA['math_random_float'] = function(block) {
  ...
  return ['Math.random()', Blockly.ASEBA.ORDER_FUNCTION_CALL];
};
